import { err, Unique } from "@tsly/core";
import { isNone, isSome } from "@tsly/maybe";
 ...
 */
export class TslyIter<T> implements Generator<T> {
  ...
  /**
   * Creates a new {@link TslyIter} from a generator factory function.
   *
   * ?> Use {@link iter} rather than calling raw constructor functions
   */
  static fromGeneratorFn<T>(factory: () => Generator<T, void, unknown>) {
    return new TslyIter(factory());
  }

  /**
   * Creates a new {@link TslyIter} from an existing iterator
   *
   * ?> Use {@link iter} rather than calling raw constructor functions
   */
  static fromIter<T>(iter: Iterator<T, void, unknown>) {
    return TslyIter.fromGeneratorFn(function* () {
      yield* { [Symbol.iterator]: () => iter };
    });
  }

  /**
   * Creates a new {@link TslyIter} from an array's internal iterator
   *
   * ?> Use {@link iter} rather than calling raw constructor functions
   */
  static fromArr<T>(arr: T[]) {
    return TslyIter.fromIter(arr[Symbol.iterator]());
  }
   ...
   * ```
   */
  map<U>(func: (val: T, idx: number) => U): TslyIter<U> {
   ...
   * ```
   */
  filter(func: (value: T) => boolean): TslyIter<T> {
    const generator = this.generator;
    return TslyIter.fromGeneratorFn(function* () {
  ...
  take(count: number): TslyIter<T> {
    ...
    return new TslyIter(_take(count));
  }
  ...
  collectInto<E>(mapping: (it: T[]) => E): E {
    return mapping(this.collect());
  }
   ...
   * @param other - Another {@link TslyIter} to chain with the current iterator.
   * @returns A new {@link TslyIter} containing elements from both iterators in sequence.
   *
   * @example
   * ```ts
   ...
   * ```
   */
  chain<U>(other: TslyIter<U>): TslyIter<T | U> {
    const generator = this.generator;
    return TslyIter.fromGeneratorFn(function* () {
      yield* generator;
   ...
   * ```
   */
  zip<U>(other: TslyIter<U>): TslyIter<[T, U]> {
    const generator = this.generator;
    return TslyIter.fromGeneratorFn(function* () {
   ...
   * ```
   */
  filterMap<U>(func: (val: T) => U | null): TslyIter<U> {
    return this.map(func).filter(isSome) as TslyIter<U>;
  }
   ...
   * ```
   */
  enumerate(): TslyIter<[number, T]> {
    return this.map((val, idx) => [idx, val]) as TslyIter<[number, T]>;
  }
   ...
   * @returns A new {@link TslyIter} containing the remaining elements after skipping.
   *
   * @example
   * ```ts
   ...
   * ```
   */
  skipWhile(func: (val: T) => boolean): TslyIter<T> {
    return TslyIter.fromGeneratorFn(
      function* (this: TslyIter<T>) {
        let skipping = true;
   ...
   * @returns A new {@link TslyIter} containing elements taken until the predicate returns `false`.
   *
   * @example
   * ```ts
   ...
   * ```
   */
  takeWhile(func: (val: T) => boolean): TslyIter<T> {
    return TslyIter.fromGeneratorFn(
      function* (this: TslyIter<T>) {
        for (const val of this) {
          if (!func(val)) return;
   ...
   * @returns A new {@link TslyIter} containing the mapped elements until a `null` value is encountered.
   *
   * @example
   * ```ts
   ...
   * ```
   */
  mapWhile<U>(func: (val: T) => U | null): TslyIter<U> {
    return TslyIter.fromGeneratorFn(
      function* (this: TslyIter<T>) {
        for (const val of this) {
   ...
   * @param func - The reducer function used to combine the elements.
   * @returns A new {@link TslyIter} containing intermediate scan values.
   *
   * @example
   * ```ts
   * iter([1, 2, 3, 4, 5])
   ...
   * ```
   */
  scan<U>(initialValue: U, func: (accumulator: U, val: T) => U): TslyIter<U> {
    return TslyIter.fromGeneratorFn(
      function* (this: TslyIter<T>) {
   ...
   *
   * @returns A new {@link TslyIter} with elements at the deepest level of nesting.
   *
   * @example
   * ```ts
   ...
   * ```
   */
  flatten(): TslyIter<T extends (infer Inner)[] ? Inner : T> {
    return TslyIter.fromGeneratorFn(
      function* (this: TslyIter<T>) {
        for (const val of this) {
   ...
   * @returns A new {@link TslyIter} containing the mapped and flattened elements.
   *
   * @example
   * ```ts
   ...
   * ```
   */
  inspect(func: (val: T) => void): TslyIter<T> {
    return this.map((val) => {
      func(val);
      return val;
    });
   ...
   * @returns A tuple of two {@link TslyIter} instances: one containing elements that satisfy the predicate, and the other containing elements that don't.
   *
   * @example
   * ```typescript
   * function isEven(val: number): boolean {
  ...
  partition(func: (val: T) => boolean): [passIter: TslyIter<T>, failIter: TslyIter<T>] {
    ...
    return [TslyIter.fromArr(a), TslyIter.fromArr(b)];
  }
   ...
   *
   * This method returns a new {@link TslyIter} with duplicate elements removed.
   *
